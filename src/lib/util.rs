...
use colorful::{self, core::color_string::CString, Color, Colorful};
...
pub fn generate(config: &Config) -> String {
    ...
    if chars.len() <= 0 {
        eprintln!("Empty Char list. Exiting!");
        std::process::exit(1)
    }
...
pub fn format_output(password: String, entropy: f64, config: &Config) -> String {
    ...
    if config.check_wordlists {
        ...
        for oc in occurrences {
            format_occurrences
                .push_str(format!("!Found in wordlist {} on line {}!\n", oc.0, oc.1).as_str());
        }
    }
...
{d}
",
        d = "-".repeat(std::cmp::min(
            password.len(),
            termion::terminal_size().unwrap().0.into()
        ))
    )
    .to_string()
}
