...
impl Config {
    pub fn new(
        len: i32,
        chars: Vec<util::Chars>,
        exclude: Option<Vec<char>>,
        check_wordlists: bool,
    ) -> Config {
        Config {
            chars: chars,
            len: len,
            exclude: exclude,
            check_wordlists: check_wordlists,
        }
    }
    ...
    pub fn parse(args: Vec<String>) -> Result<Config, ConfigError> {
        ...
        len = args
            .get(1)
            .unwrap_or(&Config::default().len.to_string())
            .parse::<i32>()
            .unwrap_or(Config::default().len);
        ...
        if args.contains(&"-p".to_string()) {
            ...
            if pool.contains(&"u".to_string()) {
                chars.push(Chars::Uppercase)
            }
            if pool.contains(&"l".to_string()) {
                chars.push(Chars::Lowercase)
            }
            if pool.contains(&"d".to_string()) {
                chars.push(Chars::Digits)
            }
            if pool.contains(&"s".to_string()) {
                chars.push(Chars::Digits)
            }
        }
        // Get excluded
...
impl std::fmt::Display for Config {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Length: {} || Characters: {:?} || Excluded: {:?} || Check Wordlists: {}",
            self.len,
            self.chars(),
            self.exclude,
...
impl Default for Config {
    fn default() -> Config {
        Config {
            chars: vec![Chars::Uppercase, Chars::Lowercase, Chars::Digits],
            len: 12,
            exclude: None,
            check_wordlists: false,
        }
    }
}
...
mod tests {
    ...
    fn test_parser() {
        ...
        assert_eq!(
            ...
            Config::new(
                16,
                vec![Chars::Uppercase, Chars::Lowercase, Chars::Digits],
                Some(vec![]),
                true
            )
        );
